...
async function fetchBibleVersesFromBibleCom() {
  ...
  async function fetchEntireBibleVersion(version: version, bible: Bible, lang: string) {
    getBibleBooksList(lang)
      .map(async book => {
        let chapters: bibleChapter[] = [];
        book.chapters
          .map(async chapterNumber => {
            if (chapterNumber.includes('INTRODUCTION') || chapterNumber.toUpperCase() === book.human.toUpperCase()) chapterNumber = 'INTRO1';
  ...
  async function fetchBookChapter(bookUsfm: string, chapterNumber: string, version: version, chapters: bibleChapter[]) {
 ...
 */
function getBibleChapterText(bible: Bible, bookName: string, chapterNumber: string): string {
  if (!bible) return;
...
/**
   ...
   * @param {string} bookName - the initials of a given book of bibleVersion
   */
function getBibleBook(bible: Bible, bookName: string): [string, string[][]][] {
...
function getBibleBooksList(lang: string): bibleBookKeys[] {
  ...
  let bookListEN = [];
  if (lang === 'AR') return bookListAR;
  else if (lang === 'FR') return bookListFR;
  else if (lang === 'EN') return bookListEN;
}

function arrangeBibleChapters(bible: Bible):Bible {
